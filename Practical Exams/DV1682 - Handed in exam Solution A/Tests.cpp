...
std::pair<int,int> MergesortTests()
{
	...
	numFailedTests += AssignmentTests(&mergeSort, algorithmName, myData, numElements, 68, 80);	// Could be 18 or 32, if they implement a tmp value to store the value to be swapped or not.
	std::sort(myData, myData+numElements);
	numFailedTests += AssignmentTests(&mergeSort, algorithmName, myData, numElements, 68, 80);
	if(numFailedTests == oldFailedTests)
		std::cout << "OK!" << std::endl;
	...
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(i);
	numFailedTests += ComparisonTests(&mergeSort, algorithmName, myData, numElements, 19, 40);
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(10 - i);
	numFailedTests += ComparisonTests(&mergeSort, algorithmName, myData, numElements, 15, 40);
	if(numFailedTests == oldFailedTests)
		std::cout << "OK!" << std::endl;
...
std::pair<int,int> SelectionsortTests()
{
	...
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(data[i]);
	numFailedTests += AssignmentTests(&selectionSort, algorithmName, myData, numElements, 18, 50);	// Could be 18 or 32, if they implement a tmp value to store the value to be swapped or not.
	std::sort(myData, myData+numElements);
	numFailedTests += AssignmentTests(&selectionSort, algorithmName, myData, numElements, 18, 50);
	if(numFailedTests == oldFailedTests)
		std::cout << "OK!" << std::endl;
	...
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(i);
	numFailedTests += ComparisonTests(&selectionSort, algorithmName, myData, numElements, 45, 90);
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(10 - i);
	numFailedTests += ComparisonTests(&selectionSort, algorithmName, myData, numElements, 45, 90);
	if(numFailedTests == oldFailedTests)
...
std::pair<int,int> InsertionsortTests()
{
	...
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(data[i]);
	numFailedTests += AssignmentTests(&insertionSort, algorithmName, myData, numElements, 44, 70);
	std::sort(myData, myData+numElements);
	numFailedTests += AssignmentTests(&insertionSort, algorithmName, myData, numElements, 9, 30);
	if(numFailedTests == oldFailedTests)
		std::cout << "OK!" << std::endl;
	...
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(i);
	numFailedTests += ComparisonTests(&insertionSort, algorithmName, myData, numElements, 9, 30);
	for(int i = 0; i < numElements; ++i)
		myData[i] = MyInt(10 - i);
	numFailedTests += ComparisonTests(&insertionSort, algorithmName, myData, numElements, 45, 70);
	if(numFailedTests == oldFailedTests)
		std::cout << "OK!" << std::endl;
...
std::pair<int,int> IterativeBinarySearchTests()
{
	const int numTests = 6;
	const int numElements = 100;
	const std::string algorithmName = "iterative Binary Search";
	std::cout << "-------" << std::endl;
	std::cout << "Starting " << algorithmName << " tests." << std::endl;
	int numFailedTests = numTests;
	...
	if(!(MyInt::numComparisons ==  6|| MyInt::numComparisons ==  12|| MyInt::numComparisons == 18))
	{
		...
		std::cout << "Should be 6/12/18" << " comparisons, but performed " << MyInt::numComparisons << " comparisons." << std::endl;
		numFailedTests -= 1;
	}
	...
	{
		std::cout << "Your " << algorithmName << " does the wrong amount of comparisons searching for " << toSearch << " in [0, 1, 2, ..., 97, 98, 99]." << std::endl;
		std::cout << "Should be 1 comparison, but performed " << MyInt::numComparisons << " comparisons." << std::endl;
		numFailedTests -= 1;
	}
	...
	{
		std::cout << "Your " << algorithmName << " does the wrong amount of comparisons searching for " << toSearch << " in [0, 1, 2, ..., 97, 98, 99]." << std::endl;
		std::cout << "Should be 7/14/21 comparisons, but performed " << MyInt::numComparisons << " comparisons." << std::endl;
		numFailedTests -= 1;
	}
