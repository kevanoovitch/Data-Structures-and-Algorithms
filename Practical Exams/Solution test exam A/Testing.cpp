...
int AssignmentTests(void (*AlgorithmFunction)(MyInt*, int), std::string algorithmName, MyInt* data, int numElements, int numAssignments)
{
	...
	if (MyInt::numAssignments != numAssignments)
	{
		std::cout << std::endl << algorithmName << " does not assign elements as defined in the algorithm description." << std::endl;
		std::cout << "Your algorithm did " << MyInt::numAssignments << ". It should do " << numAssignments << "." << std::endl;
...
int ComparisonTests(void (*AlgorithmFunction)(MyInt*, int), std::string algorithmName, MyInt* data, int numElements, int numComparisons)
{
	...
	AlgorithmFunction(toSort, numElements);
	if (MyInt::numComparisons > numComparisons)
	{
		...
		std::cout << "Your algorithm did " << MyInt::numComparisons << ". It should do " << numComparisons << "." << std::endl;
		delete[] toSort;
		return -1;
	}
	else if (MyInt::numComparisons < numComparisons)
	{
